/*
# Fix approve_deposit silent failure + robust error handling

## Problem
approve_deposit returned NULL silently when:
- admin check failed
- deposit not found
- deposit already processed
The frontend didn't check the return value, so it showed "Deposit approved"
even when nothing happened. The customer's balance/frozen never updated.

## Fix
- Raise explicit exceptions instead of returning NULL
- Frontend can now catch real errors and show them
*/
...
-- Clean up test data
DELETE FROM orders WHERE user_id = 'test-trace-user';
DELETE FROM deposits WHERE user_id = 'test-trace-user';
DELETE FROM user_profiles WHERE user_id = 'test-trace-user';
