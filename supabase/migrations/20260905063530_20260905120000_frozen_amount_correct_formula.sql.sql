/*
# Fix Frozen Amount Formula — Balance Stays Positive

## Purpose
Replace the old "negative balance" frozen model with a correct formula where:
- The customer's wallet balance NEVER goes negative.
- Frozen Amount = total_price + commission (while shortage > 0).
- Frozen Amount = 0 when the customer has enough balance to cover the order.
- On order completion: balance -= total_price, then balance += commission (net: -total_price + commission).
- No double-deduction: the price is deducted once at completion, not at submission.

## Changes

### submit_order (rewritten)
- Sufficient balance (balance >= total_price):
  - Deducts total_price from balance, adds commission.
  - frozen_amount = 0, pending_shortage = 0.
  - Order status = 'completed'.
  - Referral bonuses credited.
- Insufficient balance (balance < total_price):
  - Does NOT change balance.
  - Sets frozen_amount = total_price + commission.
  - Sets pending_shortage = total_price - balance (remaining amount needed).
  - Order status = 'pending_insufficient'.
  - No commission credited yet.
- Idempotency: if order with same order_number exists, returns current profile.

### complete_order (rewritten)
- Only works when frozen_amount = 0 (i.e. balance is sufficient).
- Deducts total_price from balance, adds commission.
- Clears frozen_amount and pending_shortage.
- Marks the existing pending_insufficient order as 'completed'.
- If the order was already completed, returns current profile (idempotent).
- Referral bonuses credited.

### approve_deposit (rewritten)
- Credits deposit amount to balance.
- Recalculates frozen_amount from the user's pending_insufficient order:
  - If balance >= required_amount: frozen = 0, shortage = 0 (order is ready).
  - If balance < required_amount: frozen = required_amount + commission, shortage = required_amount - balance.
...
-- ============ submit_order — corrected frozen formula ============
...
BEGIN
  ...
  IF v_current_balance >= p_total_price THEN
    ...
    -- Deduct price, add commission. Frozen = 0 (order completed).
  ...
  -- Balance does NOT change. Frozen = total_price + commission.
  INSERT INTO orders (
    user_id, order_number, task_number, product_id, product_name,
    merchant, unit_price, total_price, required_amount, commission, commission_rate,
    is_lucky, lucky_commission_percent, vip_level, status, note
  ...
  -- Balance stays the same. Frozen = total_price + commission.
  UPDATE user_profiles
  SET
...
-- ============ complete_order — deducts price once, adds commission ============
...
BEGIN
  ...
  -- Guard: must have sufficient balance (frozen must be 0)
  IF v_current_balance < p_total_price THEN
    RAISE EXCEPTION 'Insufficient balance to complete order. Frozen: $%, Balance: $%, Required: $%',
      v_frozen, v_current_balance, p_total_price;
  END IF;
  ...
  -- Update or insert the order row as completed
  IF FOUND THEN
  ...
  -- Deduct price once, add commission. Clear frozen.
  UPDATE user_profiles
  SET
    balance = balance - p_total_price + p_commission,
    frozen_amount = 0,
...
-- ============ approve_deposit — recalculate frozen with correct formula ============
...
BEGIN
  ...
  IF FOUND THEN
    ...
    IF v_new_balance >= v_pending_order.required_amount THEN
      ...
      v_new_shortage := 0;
    ELSE
      -- Still short — frozen = total_price + commission, shortage = required - balance
      v_new_frozen := v_pending_order.total_price + v_pending_order.commission;
      v_new_shortage := v_pending_order.required_amount - v_new_balance;
    END IF;
