/*
# Allow negative (frozen) balance in complete_order

## Purpose
Previously, `complete_order` rejected any order where the product price exceeded
the user's current balance. The new requirement is to ALLOW the order to go
through and let the balance go negative, representing the amount the customer
needs to deposit to continue.

## Changes
1. `complete_order` — removed the `RAISE EXCEPTION 'Insufficient balance'` check.
   The function now deducts `p_total_price` from `balance` unconditionally,
   then credits `p_commission`. If `p_total_price > balance`, the result is
   a negative balance (e.g. $200 - $500 = -$300).

## Security
- Function remains SECURITY DEFINER with `SET search_path TO 'public'`.
- The `FOR UPDATE` row lock is kept to prevent race conditions.
...
BEGIN
  -- Atomically lock the user's balance
  ...
  -- Deduct order price and credit commission (balance can go negative)
